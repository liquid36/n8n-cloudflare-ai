...
import { NodeConnectionTypes } from './utils';
...
export class N8nLlmTracing extends BaseCallbackHandler {
	...
	async handleLLMEnd(output: LLMResult, runId: string) {
		...
		this.executionFunctions.addOutputData(this.connectionType as any, runDetails.index, [
			[{ json: { ...response } }],
		]);
	...
	async handleLLMStart(llm: Serialized, prompts: string[], runId: string) {
		...
		const { index } = this.executionFunctions.addInputData(this.connectionType as any, [
			[
				{
					json: {
						messages: prompts,
	...
	async handleLLMError(
		...
		if (error instanceof NodeError) {
			...
			this.executionFunctions.addOutputData(this.connectionType as any, runDetails.index, error);
		} else {
			// If the error is not a NodeError, we wrap it in a NodeOperationError
			this.executionFunctions.addOutputData(
				this.connectionType as any,
				runDetails.index,
